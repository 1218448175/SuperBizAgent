...
MCP_TOOL_CALLS = Counter(
    "mcp_cls_tool_calls_total",
    "Total CLS MCP tool calls",
    ["tool_name", "status"],
)
MCP_TOOL_LATENCY = Histogram(
    "mcp_cls_tool_latency_seconds",
    "CLS MCP tool call latency",
    ["tool_name"],
)
...
def log_tool_call(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            MCP_TOOL_CALLS.labels(tool_name=method_name, status="success").inc()
            MCP_TOOL_LATENCY.labels(tool_name=method_name).observe(_elapsed)
            ...
            # 记录错误状态
            MCP_TOOL_CALLS.labels(tool_name=method_name, status="error").inc()
